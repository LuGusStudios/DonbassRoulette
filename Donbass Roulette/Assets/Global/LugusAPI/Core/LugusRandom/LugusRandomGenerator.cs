...
public abstract class ILugusRandomGenerator
{
	...
	protected System.Random RandomClass 
	{
		get 
		{
			return _r;
	...
	protected int _seed;
	protected int Seed 		// We don't want this to be public next to SetSeed().
	{
		get 
		{
			return _seed;
	...
	public virtual int GetInt(int max)
	{
		return _r.Next(max);
	}
	...
	public virtual float GetValue(float max)
	{
		return (float)_r.NextDouble() * max;
	}

	public virtual float GetValue(float min, float max)
	{
		return (float)_r.NextDouble() * (max - min ) + min;
	}

	public virtual float GetValue(DataRange dr)
	{
...
public class LugusRandomGeneratorPerlin : ILugusRandomGenerator
{
	...
	{
		return GetPerlin( base.GetValue() / _dr.from, base.GetValue() / _dr.to);
	}
