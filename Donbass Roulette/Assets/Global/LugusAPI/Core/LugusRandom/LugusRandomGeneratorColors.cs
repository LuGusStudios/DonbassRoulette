...
public class LugusRandomGeneratorColors : ILugusRandomGenerator
{
	...
	public new Color GetValue ()
	{
		return new Color(base.GetValue(), base.GetValue(), base.GetValue());
	}
	...
	public List<Color> GenerateColorsUniform(int colorCount)
	{
		...
		{
			Color newColor = new Color(GetByte(), GetByte(), GetByte());
			colors.Add(newColor);
		}
		return colors;
	}
	...
	{
		List<Color> colors = new List<Color>();
		float referenceAngle = base.GetValue() * 360;
		for (int i = 0; i < colorCount+1; i++)
		{
			float randomAngle = base.GetValue(rangeAngle0 + rangeAngle1 + rangeAngle2);
			...
			float newSaturation = saturation + (base.GetValue() - 0.5f) * saturationRange;
			float newLuminance = luminance + +(base.GetValue() - 0.5f) * luminanceRange;
