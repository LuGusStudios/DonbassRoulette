...
public class LugusRandomGeneratorDistribution : ILugusRandomGenerator {
	...
	public float GetValue(float min, float max, float delta)
	{
		return DistributionMethod(min, max, delta);
	}

	public override float GetValue (DataRange dr)
	{
		return DistributionMethod(dr.from,dr.to,_delta);
	}

	public float GetValue(DataRange dr, float delta)
	{
		return DistributionMethod(dr.from,dr.to,delta);
	}

	public float GetDoubleDistribution()
	{
		switch (_distType) 
		{
	...
	protected float ExponentialNormalized(float lambda) {
		if(lambda >= 1)
		{
			return (-Mathf.Log(1 - base.GetValue()) / lambda) / (_expLimit);// 
		}
		else
		{
			//TODO: now this will result in a graph where graph equals to lamda = 1
			return (-Mathf.Log(1 - base.GetValue()) / lambda) / ((1+_expLimit)/lambda);
		}
	}
