...
public class iTweener : MonoBehaviour 
{
    ...
    public void Execute()
    {
		...
		// When updating this, also copy it to YieldExecute below.
	...
	public Coroutine YieldExecute()
	{
		...
		// The iTween methods don't return the iTween object. Instead, we have to somehow clumsily retrieve it from _subject.
		// But there may well be more than one iTween attached to _subject.
		// The ID generated (unique at least per GameObject) below allows us to distinguish it from any other iTweens that might be running on the same object.
		
		// NOTE: This will most likely go wrong if you somehow also try to manually set the ID through the standard iTween Hashtable argument.
		// However, since this method can only be accessed through iTweener to begin with, that seems like an unlikely scenario.
		
		string yieldTweenID = "YieldITween";
		int yieldTweenCounter = 0;
		
		
		foreach(iTween tween in _subject.GetComponents<iTween>())
		{
			if (tween.id.StartsWith(yieldTweenID))
				yieldTweenCounter ++;
		}
		
		string generatedID = (yieldTweenID + yieldTweenCounter).ToString();
		
		arguments.Add("id", generatedID);

		// Make sure this is identical to the regular Execute below.
		if (type == TweenType.MoveTo)
			iTween.MoveTo(_subject, arguments);
		else if (type == TweenType.ScaleTo)
			iTween.ScaleTo(_subject, arguments);
		...
		return LugusCoroutines.use.StartRoutine(WaitForTweenFinishRoutine(_subject, generatedID, this)).Coroutine;
	}
